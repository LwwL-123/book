...
func verifyPreorder(preorder []int) bool {
    ...
    // 找右子树中有没有小于根节点的
    for i := cur+1; i < len(preorder); i++ {
        if preorder[i] < preorder[0] {
            return false
        }
    }

    return verifyPreorder(preorder[1:cur]) && verifyPreorder(preorder[cur:])
}
```
...
func verifyPostorder(postorder []int) bool {
    ...
    cur := len(postorder)-1  //初始值对应没有右子树的情况
    ...
    for i:=cur; i<len(postorder)-1; i++ {
        if postorder[i] < postorder[len(postorder)-1] { //右子树中若有节点值小于根节点则false
            return false
        }
    }  //递龟检查左子树和右子树
    return verifyPostorder(postorder[:cur]) && verifyPostorder(postorder[cur:len(postorder)-1])
}
```
...
func (this *Codec) deserialize(data string) *TreeNode {    
    ...
    var buildTree func(*[]string) *TreeNode 
    buildTree = func(nums *[]string) *TreeNode {
        rootVal := (*nums)[0]
        *nums = (*nums)[1:]
        if rootVal == "#" {
            return nil
